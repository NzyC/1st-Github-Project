...
//
// This program was written by Nahiyan Mahmud (z5419785)
// on [21/07/2022]
//
// This program uses a linked list of tables, and a sushi train made of a 
// linked list of plates to monitor a sushi train restaurant. It prompts the
// user to input commands that can manipulate these linked lists.
...
void reverse_train(struct plate** head);
void rotate_train(struct plate** head_ptr);

// TODO: Your function prototypes here
struct table *create_tables(char customer[MAX_STRING_LENGTH], 
    struct plate *orders, struct table *next);
struct table *insert_at_end(struct restaurant *sushi_restaurant, 
    struct table *head_ptr);
void insert_customer(struct restaurant *sushi_restaurant, 
    struct table *head_ptr);
struct plate* create_plate(struct plate *next);
struct plate* insert_in_sequence(struct restaurant *sushi_restaurant, 
    struct plate **head);
void print_plates(struct plate *head);
struct plate *create_plate_two(char roll_name[MAX_STRING_LENGTH],
             enum plate_colour colour, enum sushi_type type, 
    struct plate *next);
...
int main(void) {
    ...
    while (scanf_command(&command) == 1) {
        ...
        if (command == 'a') {
            // add table to end of tables linked list inside the restaurant 
            // struct
            insert_at_end(sushi_restaurant, sushi_restaurant->tables);
        }
        if (command == 'c') {      
            // prompt user to input a customers name
            // assign this customer name to the first table without a customer
            // print error message if there are no available tables
            insert_customer(sushi_restaurant, sushi_restaurant->tables);
        }
        if (command == 'r') {      
            // prompt user to input a dish name, plate colour, and sushi type
            // this order is added to the sushi train in correct position
            // sorted by type
            insert_in_sequence(sushi_restaurant, &sushi_restaurant->plates);
        }
        if (command == 't') {      
            // print out all plates in sushi train
            print_plates(sushi_restaurant->plates);
        }
        if (command == 'o') {    
            // prompts user to input a customer name and dish name
            // if the order is on the train, it's moved to the customer's table
            // if not, user is prompted to input sushi details, and is made 
            // straight into the customer's table  
            order(sushi_restaurant);
        }
        if (command == 'b') {  
            // prompts user to input a customer name
            // calculates the customers bill
            // free all the table's orders and remove customer from table    
            calculate_bill(sushi_restaurant);
        }
        if (command == 'q') {      
            // free all malloc'd memory in the sushi restaurant
            closing_restaurant(sushi_restaurant);
            printf("Thank you for dining with CSE Sushi Restaurant!\n");
            return 0;
        }
        if (command == 's') {   
            // reverse order of plates in sushi train   
            reverse_train(&sushi_restaurant->plates);
        }
        if (command == 'm') {      
            // rotates the plates on the sushi train by amount inputed by user
            rotate_train(&sushi_restaurant->plates);
        }
...
// create table, passing through customer name, orders, and pointer to next
// table

struct table* create_table(char customer[MAX_STRING_LENGTH], 
                           struct plate *orders, struct table *next) {
...
struct table *insert_at_end(struct restaurant *sushi_restaurant, 
    ...
    if (current == NULL) {
        struct table *new_table = create_table(EMPTY_CUSTOMER, NULL, NULL);
        sushi_restaurant->tables = new_table;
    } else {
        // loop through tables until an empty one is found
...
void insert_customer(struct restaurant *sushi_restaurant, 
    ...
    if (current == NULL) {
        ...
        // loop through tables until the correct customer is found or until
        // the last table
        while (strcmp(current->customer, EMPTY_CUSTOMER) == 1 
               && current->next != NULL) {
            current = current->next;
        }
...
// create plate, passing through pointer to next plate
struct plate* create_plate(struct plate *next) {
...
struct plate* insert_in_sequence(struct restaurant *sushi_restaurant, 
    ...
    struct plate *new_plate = create_plate(NULL);

    // loop through plates in sushi train until the correct position is found
    while (current != NULL && current->type <= new_plate->type) {
        previous = current;
        current = current->next;
    }
...
// another version of create_plate function, passing through the roll name,
// plate colour, sushi type and pointer to next plate
struct plate *create_plate_two(char roll_name[MAX_STRING_LENGTH],
    enum plate_colour colour, enum sushi_type type, struct plate *next) {
...
void order(struct restaurant *sushi_restaurant) {
    ...
    char order_input[MAX_STRING_LENGTH];
    fgets(order_input, MAX_STRING_LENGTH, stdin);
    remove_newline(order_input);
    
    char customer[MAX_STRING_LENGTH];
    char dish_name[MAX_STRING_LENGTH];
    interpret_order(order_input, customer, dish_name);
    
    // loop through tables until the correct customer is found
    while (current_table != NULL 
           && strcmp(current_table->customer, customer) != 0) {
        current_table = current_table->next;
    }
    
    // if the customer is not found, print error statement
    if (current_table == NULL) {
        printf("There is no customer with that name!\n");
        return;
    }
    
    // loop through sushi train until the order is found or it's at the 
    // last plate
    while (current_plate != NULL 
           && strcmp(current_plate->roll_name, dish_name) != 0) {
        previous_plate = current_plate;
        current_plate = current_plate->next;
    }
    
    struct plate *table_plate = current_table->orders;
    // if the plate is not found on the train, prompt user to enter fish details
    if (current_plate == NULL) {
        ...
        struct plate *new_plate = create_plate_two(dish_name, colour, type, NULL);
...
void calculate_bill(struct restaurant *sushi_restaurant) {
    ...
    char latest_customer[MAX_STRING_LENGTH];
    fgets(latest_customer, MAX_STRING_LENGTH, stdin);
    remove_newline(latest_customer);
    
    while (current_table != NULL) {
        if (strcmp(current_table->customer, latest_customer) == 0) {
            ...
            // go back to billed table and free all malloc'd plates
...
void closing_restaurant(struct restaurant *sushi_restaurant) {
    ...
    // loop through all tables in restaurant
    while (current_table != NULL) {
        if (current_table->orders != NULL) {
            current_plate = current_table -> orders;
            current_table->orders = NULL; 
        }
        // loop through all plates in table and free their malloc'd data
        while (current_plate != NULL) {
            previous_plate = current_plate;
            current_plate = current_plate->next; 
...
void rotate_train(struct plate** head_ptr) {
    ...
    struct plate* current = *head_ptr;

    int count = 1;
    while (count < rotate_num && current != NULL) {
        current = current->next;
        count++;
    }
    ...
    struct plate* rotating_plate = current;
    ...
    *head_ptr = rotating_plate->next;

    rotating_plate->next = NULL;
}
